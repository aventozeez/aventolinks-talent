...
import { ArrowLeft, Star, Trophy, Shuffle } from 'lucide-react'
...
export default function MentorsPage() {
  ...
  const revealAll = () => {
    ...
    const tick = () => {
      ...
      i++
      if (i < 16) setTimeout(tick, 180)
      else { setAllRevealed(true); setRevealingAll(false) }
    }
    tick()
  }
  ...
  return (
    <div className="min-h-screen bg-[#060f1f] text-white">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-8 flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              {allRevealed
                ...
                : `${revealed.filter(Boolean).length} of 16 revealed — click a card to unveil`}
            </p>
          </div>
          <div className="flex gap-3 flex-wrap">
            ...
            {allRevealed && (
              <button
                onClick={reset}
                className="flex items-center gap-2 px-5 py-2.5 bg-white/10 text-white font-semibold rounded-xl hover:bg-white/20 text-sm transition-colors border border-white/20"
              >
                🔄 Reset Cards
              </button>
            )}
        ...
        <div className="max-w-6xl mx-auto px-6 pb-4">
          <div className="w-full bg-white/10 rounded-full h-1.5">
            <div
              ...
              style={{ width: `${(revealed.filter(Boolean).length / 16) * 100}%` }}
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          {slots.map((slot, i) => {
            ...
            return (
              <div
                ...
                className={`relative cursor-pointer ${isRevealed ? 'cursor-default' : 'cursor-pointer'}`}
              >
                <div
                  style={{
        ...
        {/* ── Champion / Final summary if draw is done ── */}
        {allRevealed && (
          <div className="mt-10 bg-gradient-to-r from-[#f5a623]/10 to-[#f5a623]/5 border border-[#f5a623]/30 rounded-2xl p-6 text-center">
            ...
            <p className="text-sm text-slate-400">
              16 mentors have been randomly assigned. May the best team win! 🏆
            </p>
          </div>
        )}
      </div>
