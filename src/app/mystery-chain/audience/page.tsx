  ...
  if (!s || s.phase === 'setup') return (
    <div className="min-h-screen bg-[#0a0a1a] flex items-center justify-center">
      <div className="text-center space-y-3">
        ...
        <p className="text-white text-2xl font-bold">Mystery Chain</p>
        <p className="text-slate-500">Waiting for admin to start…</p>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-[#0a0a1a] text-white p-4 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={t.key} className={`rounded-xl p-4 text-center border transition-all ${
            ...
            {activeTeam === t.key && (
              <p className="text-purple-300 text-[10px] font-bold uppercase tracking-widest mb-1">Playing</p>
            )}
      ...
      {s.phase === 'story' && (
        <div className="flex-1 bg-purple-900/20 border border-purple-500/30 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
          <p className="text-purple-300 text-xs font-bold uppercase tracking-widest mb-3">Scenario</p>
          <p className="text-white text-lg leading-relaxed">{s.story}</p>
        </div>
      )}
      ...
      {(s.phase === 'a_playing' || s.phase === 'b_playing' || s.phase === 'c_playing') && (
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="bg-white/5 rounded-2xl p-4">
            ...
            <p className="text-center font-black text-4xl mt-2" style={{ color: timerColor }}>{fmtTime(timeLeft)}</p>
          </div>

          {/* Current Puzzle */}
          {s.currentPuzzle ? (
            <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-4">
      ...
      {s.phase === 'done' && (
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          <p className="text-[#f5a623] text-xs font-bold uppercase tracking-widest">Final Results</p>
