...
export default function EmergencyTeamAPage() {
  const [state,      setState]      = useState<EmergencyState>(defaultEmergencyState())
  const [loading,    setLoading]    = useState(true)
  const [selected,   setSelected]   = useState<string | null>(null)   // resource_id
  ...
  useEffect(() => {
    const sub = subscribeToEmergency(s => {
      setState(s)
      setLoading(false)
      if (s.phase === 'briefing') setDeployments([]) // reset on new round
    })
  ...
  const broadcastDeployments = useCallback(async (deps: Deployment[]) => {
    ...
    if (!latest) return
    const updated: EmergencyState = { ...latest, team_a_deployments: deps }
    broadcastEmergency(updated)
    await saveEmergencyState(updated)
  }, [])

  const deploy = async (incidentId: string) => {
    if (!selected || state.phase !== 'active' || state.team_a_submitted) return
    const next = [
  ...
  const submit = async () => {
    if (state.team_a_submitted) return
    setSubmitting(true)
    const latest = await getEmergencyState()
    if (!latest) { setSubmitting(false); return }
    const updated: EmergencyState = { ...latest, team_a_deployments: deployments, team_a_submitted: true }
    broadcastEmergency(updated)
    await saveEmergencyState(updated)
    setState(updated)
    setSubmitting(false)
  ...
  const submitted = state.team_a_submitted
        ...
        {phase === 'briefing' && (
          <div className="space-y-4">
            <div className="bg-[#0a1628] border border-[#f5a623]/30 rounded-2xl p-4">
              ...
              <p className="text-slate-400 text-xs mt-3">You have <strong className="text-white">3 minutes</strong> to deploy your emergency resources to the right incidents. Study the map carefully.</p>
            </div>
            ...
            <div className="bg-[#0a1628] border border-white/10 rounded-2xl p-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {RESOURCES.map(res => {
                  ...
                  return (
                    <button key={res.id}
                      onClick={() => setSelected(isSelected ? null : res.id)}
                      className={`relative flex flex-col items-center gap-1 p-3 rounded-xl border text-xs font-bold transition-all ${
                        isSelected ? 'border-[#f5a623] bg-[#f5a623]/15 scale-105' :
                        dep        ? 'border-green-500/40 bg-green-500/10 text-green-300' :
                                     'border-white/10 bg-white/5 text-slate-300 hover:bg-white/10'
                      }`}>
                      <span className="text-xl">{res.icon}</span>
                      <span className="text-[9px] leading-tight text-center">{res.label}</span>
                      {dep && <span className="text-[9px] text-green-400">→ {incLabel}</span>}
                      {isSelected && <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#f5a623] rounded-full text-[8px] text-[#0a1628] flex items-center justify-center font-black">✓</span>}
                    </button>
                  )
                })}
              </div>
        ...
        {phase === 'revealed' && state.score_a && (
          <div className="space-y-4">
            <div className="rounded-2xl p-5 text-center border-2" style={{ borderColor: `${COLOR}80`, background: `${COLOR}15` }}>
              ...
              <p className="text-7xl font-black" style={{ color: COLOR }}>{state.score_a.total}</p>
              {state.score_b && (
            ...
            <div className="bg-[#0a1628] border border-white/10 rounded-2xl p-4 space-y-2">
              ...
              {INCIDENTS.map(inc => {
                const s = state.score_a!.incident_scores.find(x => x.incident_id === inc.id)
                return (
              ...
              {state.score_a.coverage_bonus > 0 && (
                <div className="flex justify-between text-sm text-green-400 border-t border-white/10 pt-2 mt-1">
                  <span>✅ All incidents covered</span><span>+{state.score_a.coverage_bonus}</span>
                </div>
              )}
            </div>
          </div>
