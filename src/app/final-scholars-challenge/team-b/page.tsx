            ...
            {(s?.rf_phase === 'a_playing' || s?.rf_phase === 'b_playing') && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className={`bg-purple-950/40 border ${COLOR.border} rounded-2xl p-4 text-center`}>
                    <p className={`text-xs font-bold ${COLOR.text} truncate`}>{myName}</p>
                    <p className={`text-3xl font-black ${COLOR.text}`}>{s.rf_score_b}</p>
                  </div>
                </div>
              </>
            )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-[#0a1628] border border-white/10 rounded-2xl overflow-hidden">
                    <div className="grid grid-cols-3 bg-white/5">
                      ...
                      <div className="px-3 py-2 text-[9px] font-black text-green-400 uppercase tracking-widest text-center truncate">{theirName}</div>
                    ...
                    {Array.from({ length: (s.is_problems?.length ?? 2) }).map((_, i) => {
                      ...
                      return (
                        <div key={i} className="grid grid-cols-3 border-t border-white/5">
                          ...
                          <div className={`px-3 py-3 text-center text-xl font-black ${pa >= pb ? 'text-green-400' : 'text-slate-500'}`}>{pa}</div>
                          <div className={`px-3 py-3 text-center text-xl font-black ${pb >= pa ? COLOR.text : 'text-slate-500'}`}>{pb}</div>
                        </div>
                      )
                    })}
                    <div className="grid grid-cols-3 border-t-2 border-[#f5a623]/40 bg-[#f5a623]/5">
                      ...
                      <div className={`px-3 py-3 text-center text-3xl font-black ${aWins ? 'text-green-400' : 'text-white'}`}>{a}</div>
                      <div className={`px-3 py-3 text-center text-3xl font-black ${bWins ? COLOR.text : 'text-white'}`}>{b}</div>
                    </div>
                  </div>
                  <p className="text-center text-white text-lg font-black">
                    {aWins ? `🏆 ${theirName} wins the Innovation Sprint`
                      : bWins ? `🏆 ${myName} wins the Innovation Sprint`
                      : `🤝 It's a tie at ${a}`}
                  </p>
                </div>
              )
