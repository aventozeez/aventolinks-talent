...
export default function SimTeamBPage() {
  ...
  useEffect(() => { setAllocation(makeDefaultAllocation(scenario)) }, [scenario.id])
  ...
  const submitted      = state?.team_b_submitted ?? false
  ...
  return (
    <div className="min-h-screen bg-[#060f1f] text-white flex flex-col select-none">
      ...
      <div className="bg-purple-950/60 border-b border-purple-500/30 px-5 py-4 shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-purple-400 uppercase tracking-[0.3em]">Team B</p>
        ...
        {phase === 'working' && (
          <div className="mt-3">
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className={remaining < 0 ? 'text-red-400 font-bold' : 'text-purple-400'}>
                {remaining >= 0 ? `${remaining} kW remaining` : `${Math.abs(remaining)} kW over limit!`}
              </span>
            </div>
            <div className="h-3 bg-white/5 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-300 ${overloaded ? 'bg-red-500' : 'bg-purple-500'}`}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 max-w-lg mx-auto w-full space-y-4">
            ...
            <div className="bg-[#0a1628] border border-[#f5a623]/30 rounded-2xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-3 text-center">
                  <p className="text-[10px] text-purple-400 uppercase tracking-widest">Your Budget</p>
                  <p className="text-3xl font-black text-purple-400">{scenario.available_kw} kW</p>
                </div>
                <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-3 text-center">
                  <p className="text-[10px] text-red-400 uppercase tracking-widest">Total Demand</p>
                  <p className="text-3xl font-black text-red-400">{scenario.facilities.reduce((s,f)=>s+f.demand_kw,0)} kW</p>
                </div>
              </div>
              <p className="text-[11px] text-slate-400 text-center">You cannot power everything. Prioritise wisely.</p>
            </div>
        ...
        {phase === 'working' && !submitted && (
          <div className="space-y-3">
                ...
                <div key={f.id} className={`bg-[#0a1628] border rounded-2xl p-4 ${
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-[10px] text-purple-400 font-bold">+{pts} pts</p>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between text-[10px] text-slate-600 mt-1">
                    <span>0 kW</span>
                    <span className={ratio >= 1 ? 'text-purple-400 font-bold' : ''}>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <div className="flex gap-1.5">
                        {scenario.transformers.map(tx => (
                          <button key={tx.id} onClick={() => setTx(f.id, tx.id)}
            ...
            <button onClick={submit} disabled={submitting}
              className="w-full flex items-center justify-center gap-2 py-4 bg-purple-500 hover:bg-purple-400 text-white font-black rounded-2xl text-lg disabled:opacity-50 shadow-2xl shadow-purple-500/30 transition-colors">
        ...
        {phase === 'working' && submitted && (
          <div className="text-center py-16 space-y-5">
            <CheckCircle size={64} className="mx-auto text-purple-400" />
            <h2 className="text-2xl font-black text-purple-400">Solution Submitted!</h2>
        ...
        {phase === 'revealed' && state?.score_b && (
          <div className="space-y-4">
            <div className="bg-purple-500/15 border border-purple-500/40 rounded-2xl p-5 text-center">
              <p className="text-sm font-bold text-purple-400 mb-1">Team B — Final Score</p>
              <p className="text-7xl font-black text-purple-400">{state.score_b.total}</p>
              {state.score_a && (
            ...
            <div className="bg-[#0a1628] border border-white/10 rounded-2xl p-4 space-y-2">
              ...
              {scenario.facilities.map(f => {
                const fs = state.score_b!.facility_scores.find(x => x.facility_id === f.id)
                if (!fs) return null
                return (
                  <div key={f.id} className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full bg-purple-500 rounded-full" style={{ width: `${fs.ratio * 100}%` }} />
                      </div>
                    </div>
                    <span className="text-xs font-bold text-purple-400 shrink-0 w-12 text-right">+{fs.points}</span>
                  </div>
                )
              })}
              {state.score_b.efficiency_bonus > 0 && (
                <div className="flex justify-between text-xs text-green-400 border-t border-white/10 pt-2 mt-2">
                  <span>⚡ Efficiency bonus</span><span>+{state.score_b.efficiency_bonus}</span>
                </div>
              )}
              {state.score_b.overload_penalty > 0 && (
                <div className="flex justify-between text-xs text-red-400">
                  <span>⚠ Overload penalty</span><span>−{state.score_b.overload_penalty}</span>
                </div>
              )}
            </div>
          </div>
