...
export default function TeamBPage() {
  ...
  const weBuzzed   = phase === 'buzzed_b'
  const theyBuzzed = phase === 'buzzed_a'
          ...
          <div className="w-full space-y-5">
            ...
            {mode === 'buzzer' && phase === 'showing' && (
              <button
                ...
                className={`w-full py-8 rounded-2xl font-black text-2xl transition-all duration-150 ${
                  buzzed
                    ? 'bg-purple-600/40 border-2 border-purple-400/60 text-purple-300 opacity-70'
                    : 'bg-purple-500 hover:bg-purple-400 active:scale-95 text-white shadow-2xl shadow-purple-500/40 border-2 border-purple-400'
                }`}
              >
            ...
            {phase === 'revealed' && state?.last_result && state.last_result !== 'pass' && (
              <div className={`rounded-xl px-5 py-3.5 text-center font-bold text-base border ${
                ...
                {weGotIt    && `🎉 You got it! +${POINTS} points`}
